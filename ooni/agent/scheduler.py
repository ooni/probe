...
class SchedulerService(service.MultiService):
    ...
    def _task_did_not_run(self, failure, task):
        """React in case of abstent task.

        TODO, debug, and write the content of the debugging."""
        failure.trap(DidNotRun)
        log.debug("Did not run {0}".format(task.identifier))

    def _task_failed(self, failure, task):#may this place be the right place to raise error as described in the doc?
        """React in case of task failure during the launching.
